...
module.exports = function(req,res,next){
    pool.getConnection(function(err,connection) {//连接数据库
        ...
        if (err){//连接失败
            ...
            sqlApi(req,connection,'insert into `information_help` (information_type,information_title,information_content,information_money,information_user,information_endtime,information_phone,information_endmouthtime) values("${req.body.information_type}","${req.body.information_title}","${req.body.information_content}","${req.body.information_money}","${req.body.information_user}","${req.body.information_endtime}","${req.body.information_phone}","${req.body.information_endmouthtime}");')//使用封装的sqlAPI，第三个参数是sql语句
